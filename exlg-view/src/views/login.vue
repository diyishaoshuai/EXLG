<template>
  <div class="login-wrap">
    ...
    >
      ...
      <el-link type="primary" style="margin-left: 155px" @click="regist"
        >没有账号?去注册</el-link
      >
...
export default {
  ...
  data() {
    return {
      ...
      //rules前端验证
      rules: {
        loginInfo: [{ required: true, message: "请输入账号", trigger: "blur" }],
        password: [{ required: true, message: "请输入密码", trigger: "blur" }],
        code: [{ required: true, message: "请输入验证码", trigger: "blur" }],
      },
    };
  },
  // 创建完毕状态(里面是操作)     sadasdasd
  created() {
    this.$message({
      message: "欢迎来到EXLG-卓越物流管理平台",
      type: "success",
    });
  },
  // 里面的函数只有调用才会执行
  methods: {
    ...
    submitForm() {
      this.logining = true;
      //调用 ../api/userMG 下的login的api
      Axios({
        method: "post",
        url: `${baseUrl}/employee/empLogin`, //员工登录的请求
        data: {
        ...
        if (res.data.code === 0) {
          ...
          this.logining = false;
          //将用户信息保存到vuex中,供后续访问
          localStorage.setItem('userInfo', JSON.stringify(res.data.emp));
          // this.$store.state.emp = res.data.emp;
        } else {
          //登录失败  弹窗提示用户
          this.logining = false;
          this.$message({
